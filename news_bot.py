...
1. 유사 보도 매체 수가 높은 것(화제성 큼) + 임팩트가 큰 사건을 우선 고려해서, 가장 핫한 뉴스 5개를 선별하세요.
2. 각 뉴스마다 아래 형식을 그대로 지키세요:
   - 제목: 팩트에 기반하되, 클릭하고 싶게 만드는 자극적이고 흥미로운 한 줄 제목 (낚시성 거짓은 금지, 사실 왜곡 없이 호기심을 자극하는 표현 사용)
   - 요약: 핵심만 한 문장으로 짧게
   - 인사이트: 아래 4가지 관점 중 이 기사에 가장 잘 맞는 1~2개를 골라, "그래서 무엇이 중요한가"를 2줄로 짚어주세요.
     (a) 파급력: 이 일이 다른 산업/국가/사람들에게 어떤 연쇄 효과를 일으킬지
     (b) 숨은 맥락: 표면적 발표 뒤에 숨겨진 진짜 의도나 배경
     (c) 선례 비교: 과거 비슷한 사례와 비교했을 때 이번 건이 특별한 이유
     (d) 다음 단계: 이 사건 다음에 일어날 가능성이 높은 일
     인사이트는 기사 내용에 근거해 짧고 통찰력 있게, 추측은 "~할 가능성" 등으로 신중하게 표현하세요.
   - 출처는 원문 그대로 표기 (예: [TechCrunch], [Reuters])
   - 원문 링크 포함
3. 트렌드 정리, 그 외 부가 설명은 생략하세요.

전문 용어는 한국어로 자연스럽게 번역하고, 각 항목을 간결하게 써서 5개 항목이 모두 들어가도록 하세요.
"""
